...
def mixscape_heatmap(
    ...
    **kwds,
    ...
    """Heatmap plot using mixscape results. Need to run `pt.tl.mixscape` first.
    ...
    sc.tl.rank_genes_groups(adata_subset, layer=layer, groupby=labels, method="wilcoxon")
    sc.pp.scale(adata_subset, max_value=vmax)
    sc.pp.subsample(adata_subset, n_obs=900)
