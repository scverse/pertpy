...
class JaxSCGEN(JaxTrainingMixin, BaseModelClass):
    ...
    def predict(
        ...
        predicted_cells = self.module._bound_module.generative(stim_pred)["px"]
    ...
    def batch_removal(self, adata: AnnData | None = None) -> AnnData:
        ...
        if len(not_shared_ct) < 1:
            corrected = AnnData(
                np.array(self.module._bound_module.generative(all_shared_ann.X)["px"]),
            ...
            corrected = AnnData(
                np.array(self.module._bound_module.generative(all_corrected_data.X)["px"]),
