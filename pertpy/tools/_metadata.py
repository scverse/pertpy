    ...
    def annotate_cell_lines(
        ...
        if cell_line_type in self.cell_line_meta.columns:
            if cell_line_information is None:
                adata_copy.obs = adata_copy.obs.reset_index().merge(self.cell_line_meta, left_on=cell_line_column, right_on=cell_line_type, how='left', suffixes=('', '_fromMeta')).filter(regex='^(?!.*_fromMeta)').set_index(adata_copy.obs.index.names)
                ...
                adata_copy.obs = adata_copy.obs.reset_index().merge(cell_line_meta_part, left_on=cell_line_column, right_on=cell_line_type, how='left', suffixes=('', '_fromMeta')).filter(regex='^(?!.*_fromMeta)').set_index(adata_copy.obs.index.names)
                if cell_line_column != cell_line_type:
                    del adata_copy.obs[cell_line_type]
            else:
