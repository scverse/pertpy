...
from rpy2.robjects.packages import PackageNotInstalledError, importr, STAP
...
class Milopy:
    ...
    def annotate_nhoods(
        ...
        anno_count = adata.obsm["nhoods"].T.dot(
            csr_matrix(anno_dummies.values))
        anno_frac = np.array(anno_count/anno_count.sum(1))

        anno_frac_dataframe = pd.DataFrame(anno_frac,
                                columns=anno_dummies.columns,
                                index=milo_mdata['samples'].var_names
                                )
        milo_mdata['samples'].varm["frac_annotation"] = anno_frac_dataframe.values
        milo_mdata['samples'].uns["annotation_labels"] = anno_frac_dataframe.columns
        milo_mdata['samples'].uns["annotation_obs"] = anno_col
        milo_mdata['samples'].var["nhood_annotation"] = anno_frac_dataframe.idxmax(1)
        milo_mdata['samples'].var["nhood_annotation_frac"] = anno_frac_dataframe.max(1) 
    ...
    def annotate_nhoods_continuous(
        ...
        anno_val = adata.obsm["nhoods"].T.dot(
            csr_matrix(adata.obs[anno_col]).T)
