...
class Milo:
    ...
    def da_nhoods(
        ...
        if solver == "edger":
            ...
            if model_contrasts is not None:
                ...
                with localconverter(ro.default_converter + pandas2ri.converter + numpy2ri.converter):
                    res = base.as_data_frame(
                        edgeR.topTags(
                            edgeR.glmQLFTest(fit, contrast=mod_contrast),
                            sort_by="none",
                            n=np.inf,
                        )
                    )
            ...
            design_df_filtered = design_df_filtered.astype(
                dict.fromkeys(
                    design_df_filtered.select_dtypes(exclude=["number"]).columns,
                    "category",
                )
            )
    ...
    def plot_nhood_annotation(  # pragma: no cover # noqa: D417
        ...
        **kwargs,
        ...
        vmax = np.nanmax(
            [
                nhood_adata.obs["graph_color"].max(),
                -nhood_adata.obs["graph_color"].min(),
            ]
        )
        vmin = -vmax
    ...
    def plot_da_beeswarm(  # pragma: no cover # noqa: D417
        ...
        *,
        ...
        plt.legend(
            loc="upper left",
            title=f"< {int(alpha * 100)}% SpatialFDR",
            bbox_to_anchor=(1, 1),
            frameon=False,
        )
    ...
    def group_nhoods(
        ...
        Returns:
            None: The function modifies `adata` in place by adding the column
            `"nhood_groups"` to `adata.var`, containing the cluster labels for
            each neighborhood. Neighborhoods not included in `subset_nhoods` are
            assigned `pd.NA`.
    ...
    def _nhood_labels_to_cells_exclude_overlaps(
        ...
        mdata["rna"].obs.loc[
            ~(mdata["rna"].obs[[f"in_nhoods_{g}" for g in groups]] > min_n_nhoods).any(axis=1),
            "nhood_groups",
        ] = np.nan
        ...
        if group_to_compare is not None and baseline is not None:
            ...
            if group_to_compare is not None and baseline is not None:
                ...
                sample_obs[nhood_group_obs] = pd.Categorical(
                    sample_obs[nhood_group_obs].values,
                    categories=[baseline, group_to_compare],
                )
    ...
    def plot_heatmap_with_dot_and_colorbar(
        ...
        cbar = fig.colorbar(
            ...
            ticks=np.linspace(vmin, vmax, num=cbar_tick_count),
        )
