...
import pertpy
from pertpy.cli.custom_cli.levenstein_distance import most_similar_command

# pertpy's main commands
MAIN_COMMANDS = {"read"}
...
class HelpErrorHandling(click.Group):
    ...
    def main_options(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        ...
        """
        pertpy_main_options: List = []
        # NOTE: this only works for options as arguments do not have a help attribute per default
        for p in ctx.command.params:
            pertpy_main_options.append(("--" + p.name + ": ", p.help))  # type: ignore
        pertpy_main_options.append(("--help: ", "   Get detailed info on a command."))
        with formatter.section(HelpErrorHandling.get_rich_value("Options")):
            for t in pertpy_main_options:
                formatter.write_text(f"{t[0] + t[1]}")

    def format_help(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        """Call format_help function with pertpys customized functions.
    ...
    def format_usage(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        ...
        formatter.write_text(
            f'{HelpErrorHandling.get_rich_value("Usage:")} pertpy {" ".join(super().collect_usage_pieces(ctx))}'
        )
    ...
    def format_commands(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        ...
        with formatter.section(HelpErrorHandling.get_rich_value("Commands to work with ECCITE-Seq data")):
            formatter.write_text(
        ...
        with formatter.section(HelpErrorHandling.get_rich_value("Examples")):
            formatter.write_text("$ pertpy read data.h5ad")

        with formatter.section(HelpErrorHandling.get_rich_value("Learn more")):
            formatter.write_text(
                "Use pertpy <command> --help for more information about a command."
                "You may also want to take a look at our docs at https://pertpy.readthedocs.io/."
            )

        # with formatter.section(HelpErrorHandling.get_rich_value("Feedback")):
        #     formatter.write_text(
        #         "We are always curious about your opinion on pertpy. Join our Discord at "
...
class CustomHelpSubcommand(click.Command):
    ...
    def format_help(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        ...
        Use the overwritten format functions this class provides to output help for each subcommand pertpy provides.
    ...
    def format_usage(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        ...
        """
        formatter.write_text(
            f'{HelpErrorHandling.get_rich_value("Usage: ")}pertpy {self.name} {" ".join(self.collect_usage_pieces(ctx))}'
        )
...
def print_pertpy_version(ctx, param, value) -> None:
    """Print pertpy version styled with Rich.
    ...
    if not value or ctx.resilient_parsing:
        return
    try:
        print(f"[bold blue]pertpy version: {pertpy.__version__}")
        ctx.exit()
    except click.ClickException:
        ctx.fail("An error occurred fetching pertpy's version!")
